...
C_SLATE = (90, 90, 120)      # 2 настройки
C_BLUE = (0, 122, 255)       # 3 добавить профиль
C_PURPLE = (150, 80, 200)    # 4 название/дата подписки
C_ORANGE = (230, 140, 20)    # 5 обновить список
C_RED = (220, 60, 60)        # 6 пинг (недоступность = тревога)
C_TEAL = (20, 150, 150)      # 7 трафик
C_INDIGO = (70, 70, 200)     # 8 профили/страны
...
callouts = [
    (cx, cy, '1', C_GREEN),
    (gear_x, gear_y, '2', C_SLATE),
    (plus_x, plus_y, '3', C_BLUE),
    (25, row_y + 20, '4', C_PURPLE),
    (W - 143, icon_y + 17, '5', C_ORANGE),
    (W - 203, icon_y + 17, '6', C_RED),
    (W // 2, traf_y - 18, '7', C_TEAL),
    (77, list_y + 40, '8', C_INDIGO),
]
...
# ── Легенда ───────────────────────────────────────────────────────────────
legend = [
    ('1', C_GREEN, 'Кнопка подключения/отключения VPN'),
    ('2', C_SLATE, 'Настройки приложения'),
    ('3', C_BLUE, 'Добавить новый профиль (подписку) по ссылке или QR'),
    ('4', C_PURPLE, 'Название подписки и дата, до которой она действует'),
    ('5', C_ORANGE, 'Обновить список серверов подписки'),
    ('6', C_RED, 'Пинг — проверяет доступность и скорость сервера. Если '
                 'вместо цифры показывает «Н/Д» — сервер сейчас недоступен'),
    ('7', C_TEAL, 'Использовано трафика / доступно (∞ = безлимит)'),
    ('8', C_INDIGO, 'Профили: Auto сам выбирает лучший сервер для дома/моб. '
                    'сети; страны ниже (Finland, Sweden, Moscow) — это тоже '
                    'профили, просто с ручным выбором локации'),
]
