...
# Заменить или добавить строку KEY=VALUE в файл
set_env() {
...
if [[ -d "$INSTALL_DIR" ]]; then
    ...
    if [[ "$_ans" =~ ^(да|yes|y|д)$ ]]; then
        ...
        log_error "Удалите $INSTALL_DIR вручную для переустановки"
        exit 1
    fi
fi
...
echo -e "  Укажите ID группы/канала (начинается с -100...)"
echo -e "  Эта группа используется для всех уведомлений: покупки, бэкапы, отчёты, логи"
echo ""
ask NOTIF_CHAT_ID "* ID группы (например: -1001234567890)"

echo ""
echo -e "  Топики (тематические разделы группы). Enter = дефолт:"
ask TOPIC_GENERAL "  Общий топик — покупки/продления/триалы/баланс" "2"
ask TOPIC_INFRA   "  Инфраструктура — ошибки/серверы/подозрения"    "4"
ask TOPIC_REPORTS "  Отчёты — ежедневная статистика"                 "6"
ask TOPIC_TICKETS "  Тикеты — поддержка пользователей"               "11"
ask TOPIC_BACKUP  "  Бэкапы и логи — архивы"                         "13"
...
log_ok "POSTGRES_PASSWORD, REMNAWAVE_WEBHOOK_SECRET, CABINET_JWT_SECRET — сгенерированы"
...
# ── Remnawave Webhooks (входящие события) ─────────────────────
...
# Уведомления пользователям от вебхуков
...
# ── Web API (для приёма вебхуков платёжных систем) ────────────
...
log_ok ".env создан: $INSTALL_DIR/.env"
...
# Подключает бота к сети Remnawave Panel.
# Бот доступен из remnawave-network по имени "remnawave_bot".
networks:
  remnawave-network:
...
if [[ ! -f "$REMNAWAVE_ENV" ]]; then
    log_warn "Файл $REMNAWAVE_ENV не найден — пропускаю автонастройку вебхука"
    log_warn "Пропишите вручную в $REMNAWAVE_ENV:"
    ...
    cp "$REMNAWAVE_ENV" "${REMNAWAVE_ENV}.bak.$(date +%F-%H%M%S)"
    log_ok "Бэкап: ${REMNAWAVE_ENV}.bak.$(date +%F-%H%M%S)"
    ...
    log_ok "Вебхук прописан в $REMNAWAVE_ENV"
...
for i in {1..12}; do
    sleep 5; echo -n "."
    status=$(cd "$INSTALL_DIR" && docker compose ps --format '{{.Service}} {{.State}}' 2>/dev/null | grep '^bot' | awk '{print $2}' || true)
...
echo -e "  Webhook URL: ${CYAN}http://remnawave_bot:8080/remnawave-webhook${NC}"
echo ""
...
echo -e "  Обновление:  ${CYAN}cd ${INSTALL_DIR} && git pull && docker compose up -d --build${NC}"
echo ""
echo -e "${BOLD}Следующие шаги:${NC}"
echo -e "  1. Напишите боту /start в Telegram"
echo -e "  2. Создайте тарифы в Remnawave Panel → Тарифы"
echo -e "  3. Настройте платёжную систему в ${CYAN}${INSTALL_DIR}/.env${NC}"
echo -e "     (YOOKASSA_ENABLED=true / TELEGRAM_STARS_ENABLED=true и т.д.)"
echo ""
